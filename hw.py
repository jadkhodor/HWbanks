...
class Account:
    ...
    def deposit(self, amount):
        ...
        if type(amount) not in [int, float] or amount < 0:
            print("Incorrect amount,you can only deposit amounts above 0. Please try again!")
        else:
            self._balance += amount
            print("The following amount: {}  has been deposited to the following account : {}. Current balance is: {} . Thank you!".format(amount, self.__repr__(),
                                                                                    self._balance))
    ...
    def charge(self, amount):
        ...
        if type(amount) not in [int, float] or amount < 0 or self._balance - amount < 0:
            print("Incorrect amount! This may be due to insufficient funds, or incorrect amount requested, kindly recheck your balance and try again.")
        else:
            self._balance -= amount
            print("The following amount: {} has been deducted from {}. Current balance is: {}".format(amount, self.__repr__(),
                                                                                  self._balance))
            operation_successful = True
        return operation_successful
...
class Bank:
    ...
    def transfer(self, from_acc_id, to_acc_id, amount):
        if from_acc_id in self.account_list and to_acc_id in self.account_list:
            if from_acc_id.charge(amount) : to_acc_id.deposit(amount)
        else:
            print("You can only transfer amounts to accounts managed by this bank.")
